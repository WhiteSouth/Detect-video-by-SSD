...
from notebooks import visualization    #visualization
...
# Restore SSD model.
ckpt_filename = '../log/model.ckpt-227363'   #可更改为自己的模型路径
...
#cap = cv2.VideoCapture('japan.avi')
...
size = vid.get_meta_data()['size'] 
#fourcc = cap.get(cv2.CAP_PROP_FOURCC) 
#fourcc = cv2.CAP_PROP_FOURCC(*'CVID') 
print('fps=%d,size=%r'%(fps,size))
delay=30/int(fps)
...
img=mpimg.imread(image_path+'000001.jpg')
#print(img)
#rclasses, rscores, rbboxes =  process_image(img)
#visualization.plt_bboxes(img,rclasses,rscores,rbboxes) 
...
for num,im in enumerate(vid):
    ...
    imageio.imwrite(str(num)+'.jpg',im)


    image_np = mpimg.imread(str(num)+'.jpg')
